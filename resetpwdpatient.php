<?php
  ...
  // check whether password and confirm password are same or not
    ...
    if($pass_err == false){
    $sql=mysqli_query($con,"UPDATE patientregistration SET password='$password' WHERE email='$email'");
      if($sql == true){
             $reset=true;
           header("location:loginpatient.php?success");
        }
       else{
           header("location:resetpwdpatient.php?error");
       }
    }
    //close the session
    // session_destroy();
...
<head>
    ...
    <link rel="stylesheet" href="resetpwdpatient.css">
</head>
<body>
<div class="simple-form bgstyle" align="center">
        <h4 class="rtop">Reset Password</h4>
        <div class="container">
            <form class="mtop" action="resetpwdpatient.php" method="POST">
