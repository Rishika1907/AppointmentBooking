<?php
  ...
  // check whether password and confirm password are same or not
    ...
    if($pass_err == false){
    $sql=mysqli_query($con,"UPDATE doctorregistration SET password='$password' WHERE email='$email'");
      if($sql == true){
             $reset=true;
           header("location:doctorlogin.php?success");
        }
       else{
           header("location:resetpwddoctor.php?error");
       }
    }
    //close the session
    // session_destroy();
...
<div class="simple-form bgstyle" align="center">
        ...
        <div class="container">
            <form class="mtop" action="resetpwddoctor.php" method="POST">
